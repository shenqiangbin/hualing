DateInput = (function($) { // Localise the $ function
    ...
    DateInput.prototype = {
      ...
      '</p>');
            ...
            var todayNav = $('<p class="today_nav"><span date="' + this.dateToString(new Date()) + '">今天</span></p>');
            $(todayNav).click(this.bindToObj(function(event) {
                this.changeInput($(event.target).attr("date"));
            }));

            var nav = $('<div class="nav"></div>').append(monthNav, yearNav, todayNav);
        ...
        stringToDate: function(string) {
            var matches;
            if (matches = string.match(/^(\d{4})-(\d{1,2})-(\d{1,2})$/)) {
